...
/// "30" -> "tiga kosong" — digit-by-digit for dashed/phone groups.
fn digits_to_words(raw: &str) -> String {
    raw.chars().filter(|c| c.is_ascii_digit()).map(digit_word).collect::<Vec<_>>().join(" ")
}
...
pub fn normalize_malay(text: &str) -> String {
    ...
    // dashed digit groups -> digit words (03-8888 -> kosong tiga lapan lapan lapan)
    let t = RE_DASHED.replace_all(&t, |c: &fancy_regex::Captures<str>| {
        // python iterates chars and maps only digits — '-' maps to itself,
        // then the final join keeps single spaces between digit words.
        let words: Vec<&str> = c[1].chars().filter(|ch| ch.is_ascii_digit()).map(digit_word).collect();
        words.join(" ")
    });
